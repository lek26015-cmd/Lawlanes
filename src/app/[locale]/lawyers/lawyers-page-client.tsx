...
export function LawyersPageClient({ initialLawyers, initialRegistryLawyers }: LawyersPageClientProps) {
  ...
  // of being fetched client-side on mount — see LAWSLANE-PLAN-01 2.11.
  const [allLawyers] = useState<LawyerProfile[]>(() => sortLawyers(initialLawyers));
  const [filteredLawyers, setFilteredLawyers] = useState<LawyerProfile[]>(() => sortLawyers(initialLawyers));
  const [registryLawyers] = useState<RegistryLawyer[]>(initialRegistryLawyers);
  ...
  useEffect(() => {
    if (!matchIds || matchIdArray.length === 0) return;
  ...
  }, [matchIds, matchIdArray, allLawyers]);

  // Legacy specialty-based sorting (backward compat)
  useEffect(() => {
    if (!specialties || matchIds) return;
    ...
    return () => { isMounted = false; };
  }, [specialties, allLawyers, specialtyArray, matchIds]);
